...
class Scraper {
  ...
  async collect() {
    ...
    this.spinner.start(`Searching for ${kleur.magenta(`${this.keyword}`)}`);
    ...
    log.info(kleur.magenta(seedTitles.join('\n')));
    log.info('\n');
  ...
  private async getRecommendationsFor(video: Video | RecommendedVideo, depth = 0) {
    ...
    this.spinner.prefixText = `${kleur.gray(`${depthSign}`)}`;
    this.spinner.start(`${kleur.gray(`${ytId} |`)} ${kleur.bold().blue(`${title}`)}`);
